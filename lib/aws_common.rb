    ...
    account_ids.each do |account_id|
      regions.keep_if {|key, value| value }.keys.each do |region|
        ...
        ec2.instances.each do |instance|
          instances[instance.id] = {type: instance.instance_type, az: instance.placement.availability_zone, tenancy: instance.placement.tenancy, platform: instance.platform, account_id: account_id[0], vpc: instance.vpc_id} if instance.state.name == 'running'
        end
      end
    end
          ...
          reserved_instance.reserved_instances.each do |ri|
            if ri.state == 'active'
              ...
              if supported_platforms[account_id[0]] == 'Classic'
                instances[ri.reserved_instances_id][:vpc] = ri.product_description.include?("Amazon VPC") ? 'VPC' : ''
              else
                instances[ri.reserved_instances_id][:vpc] = 'VPC'
              end
              if ri.product_description.include? "Linux/UNIX"
                instances[ri.reserved_instances_id][:platform] = ''
